...
def trace_pathfinding(start: Tuple[int, int] = (1, 1), goal: Tuple[int, int] = (8, 7)):
    from jacobs_lab.instruments.pathfinding_lab import LAT, astar, geodesic
...
def trace_three_body(periods: int = 2, sample_every: int = 20):
    from jacobs_lab.instruments.three_body_lab import (
        FIG_V,
        FIG_X,
        PERIOD,
        energy,
...
def trace_fold_codec(kind: str = "palindrome"):
    ...
    from jacobs_lab.instruments.fold_codec import codec_bits, decode, encode, lifted_advance, raw_bits
...
def trace_fold_complexity():
    from jacobs_lab.instruments.fold_complexity import _corpora, complexity_split, fold_complexity
...
def trace_prime_machinery(limit: int = 30, vm_limit: int = 10):
    from jacobs_lab.instruments.prime_machinery import (
        folding_mod3,
        is_admissible,
        lucas_certificate,
        sieve,
...
def trace_universality_probe(max_functions: int = 12):
    from jacobs_lab.computation.universality_probe import (
        LEVEL_DOMAIN,
        ROOT_DOMAIN,
        lookup_program,
        probe_pure_folds,
