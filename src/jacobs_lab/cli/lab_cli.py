...
def cmd_trace(args):
    ...
    if args.inspect:
        try:
            from jacobs_lab.viz.lab_inspector import show_lab_trace
...
def cmd_sonify(args):
    try:
        from jacobs_lab.audio.lab_sonify_trace import write_trace_wav
    except ImportError as exc:
        raise SystemExit(
...
def cmd_test(args):
    ...
    try:
        try:
            from jacobs_lab.audio import lab_sonify_trace
        except ImportError:
            from jacobs_lab.audio import lab_sonify_trace
